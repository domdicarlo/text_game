...
# The player LOG keeps track of the player's progress throughout the game,
# and records events. TURN keeps track of how many turns have passed. a turn
# is defined as movement by the player
LOG = "\nPLAYER LOG\n\n"
TURN = -1
...
class Game(Base):
  ...
  def run(self , update_func = False):
    ...
    actor = self.player
    while True:
      # if the actor moved, describe the room, and add to turn counter
...
class Location(Base):
  ...
  # description: full description
  # event: upon first entering, the event that occurs (string only for now)
  # contents: things that are in a location
  # l_w: length and width of a location. in string(Int-Int) format
  # exits: ways to get out of a location
  # first_time: is it the first time here?
  # actors: other actors in the location

  def __init__( self, name, description, event, l_w):
    Base.__init__(self, name)
  ...
  def describe( self, observer, force=False, item=False ):
    desc = ""   # start with a blank string

    # add the description to screen and log
    if self.first_time:
      desc += (self.description_str(self.description) + '\n\n' +
          self.description_str(self.event))
      global LOG
      LOG += ('Turn ' + str(TURN) + ': ' + self.description_str(self.event) +
              '\n\n')
...
class Actor(Base):
  ...
  def __init__( self, name, player = False ):
    ...
    self.verbs.update(dict.fromkeys(['grab', 'procure', 'get', 'take'],
                                    act_multi(self.act_take1)))
    self.verbs.update(dict.fromkeys(['drop', 'let go of', 'release'],
                                    act_multi(self.act_drop1)))
    self.verbs.update(dict.fromkeys(['i', 'inventory', 'bag'],
                                    self.act_inventory))
    self.verbs.update(dict.fromkeys(['search', 'look around', 'investigate'],
                                    self.act_search))
    self.verbs.update(dict.fromkeys(['go', 'walk', 'travel'],
                                    self.act_go1))
    self.verbs.update(dict.fromkeys(['verbs', 'man', 'manual', 'help', 'l',
                                     'commands'],
                                    self.act_list_verbs))
    self.verbs.update(dict.fromkeys(['location', 'area'],
                                    self.act_location))
    self.verbs.update(dict.fromkeys(['log', 'journal', 'history'],
                                    self.act_log))
  ...
  # print out the player's log
  def act_log( self, actor, noun, words ):
    global LOG
    self.output( "%s" % LOG, FEEDBACK )
    return True
  ...
  def act_search( self, actor, noun, words ):
    print(self.location.describe( actor, False, True ))
    return True

  def act_location( self, actor, noun, words ):
    print(self.location.describe( actor, True, False ))
    return True
  ...
  def act_list_verbs( self, actor, noun, words ):
    ...
    self.output( (lambda x: x)("\n".join(sorted(self.verbs.keys()))), FEEDBACK )
    return True
...
# this makes the text search nice in the terminal... WITH COLORS!
